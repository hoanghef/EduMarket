...
filesRouter.post('/:id/download-token', requireAuth, requireCustomer, async (req, res, next) => {
  try {
    const { token, expiresAt } = await createDownloadToken(req.user.id, req.params.id);
